...
from SAC import SAC
from QEX_SAC import QEX_SAC
from utils import ExperienceReplayBuffer, NoisyExperienceReplayBuffer


# SAC tuned hyper-parameters are based on the setting outlined in the original paper: https://arxiv.org/abs/1801.01290
def hyper_param_dict(args):
    if args.env == "Ant-v2" or args.env == "HalfCheetah-v2" or args.env == "HumanoidStandup-v2" or args.env == "Humanoid-v2" or args.env == "Walker2d-v2":
        args.policy_type = "Deterministic"

    if args.env != "HalfCheetah-v2" or args.env != "Swimmer-v2":
        args.hard_update = True

    if args.env == "HumanoidStandup-v2":
    ...
    if args.hard_update:
        ...
        if args.env != "Humanoid-v2" and args.env != "HumanoidStandup-v2":
            args.updates_per_step = 4

    if args.env == "Ant-v2" or args.env == "HalfCheetah-v2":
...
def evaluate_policy(agent, env_name, seed, qex, eval_episodes=10):
...
parser = argparse.ArgumentParser(description='SAC and its QEX Implementation')

parser.add_argument('--policy', default="QEX_SAC", help='Algorithm (default: QEX_SAC)')
...
if args.policy == "SAC":
    ...
    qex = False
elif args.policy == "QEX_SAC":
    agent = QEX_SAC(env.observation_space.shape[0], env.action_space, max_action, args, device)
...
evaluations = [f"HOST: {socket.gethostname()}", f"GPU: {torch.cuda.get_device_name(args.gpu)}",
               evaluate_policy(agent, args.env, args.seed, qex)]
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        if total_numsteps % args.eval_freq == 0:
            evaluations.append(evaluate_policy(agent, args.env, args.seed, qex))
