...
# DDPG tuned hyper-parameters are imported from the paper: https://arxiv.org/abs/1509.02971
# TD3 tuned hyper-parameters are imported from the repo: https://github.com/sfujim/TD3
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DDPG, TD3 and their QEX Implementation')

    parser.add_argument("--policy", default="QEX_TD3", help='Algorithm (default: QEX_TD3)')
    ...
    elif args.policy == "QEX_DDPG":
        qex = True
        kwargs["exp_regularization"] = args.exp_regularization
        agent = QEX_DDPG.QEX_DDPG(**kwargs)
    elif args.policy == "QEX_TD3":
        qex = True
        kwargs["exp_regularization"] = args.exp_regularization
        agent = QEX_TD3.QEX_TD3(**kwargs)

    if args.load_model != "":
        policy_file = file_name if args.load_model == "default" else args.load_model
        agent.load(f"./models/{policy_file}")

    replay_buffer = utils.NoisyExperienceReplayBuffer(state_dim, action_dim, device) if qex \
    ...
    for t in range(int(args.max_time_steps)):
        ...
        if t < args.start_time_steps:
            ...
            noise = agent.generate_noise(state) if qex else np.random.normal(0, max_action * args.exp_regularization,
                                                                                  size=action_dim)
